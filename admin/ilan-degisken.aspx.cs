...
{
    public partial class ilan_degisken : System.Web.UI.Page
    {
        ...
        protected void Sayfalama()
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL = db.tbl_ilan_degisken.Where(b => b.ust_id == 0).Count();
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL = (from a in db.tbl_ilan_degisken
                           ...
                           select new
                           {
                               ...
                               a.baslik,
                               a.onay,
                               DegerSayi = db.tbl_ilan_degisken.Where(b => b.ust_id == a.id).Count(),
                               Kategori = db.tbl_ilan_degisken.Where(b => b.id == a.ust_id).Select(b => b.baslik).FirstOrDefault(),
                               a.tarih_ek,
                               a.tarih_gun,
        ...
        protected void kayitlar_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                e.Row.Cells[5].Text = "<a href=\"ilan-degisken-duzenle.aspx?ID=" + e.Row.Cells[1].Text + "\"><img src=\"img/komut-duzenle.png\" /></a>";
            }
        }
        ...
        protected void secilenlerisil_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow satir in kayitlar.Rows)
            {
                ...
                if (kutu.Checked)
                {
                    ...
                    using (BaglantiCumlesi db = new BaglantiCumlesi())
                    {
                        tbl_ilan_degisken TblKullanici = db.tbl_ilan_degisken.First(a => a.id == ID);
        ...
        {
            Response.Redirect("ilan-degisken-ekle.aspx");
        }
        ...
        protected void kayitlar_RowCommand(object sender, GridViewCommandEventArgs e)
        {
                        ...
                        foreach (var item in SQL)
                        {
                            switch (item.onay)
                            {
                                case true:
                                    ...
                                    {
                                        tbl_ilan_degisken TblOnay = dbOnay.tbl_ilan_degisken.First(a => a.id == ID);
                                        TblOnay.onay = false;
            ...
            if (e.CommandName == "Sil")
            {
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    tbl_ilan_degisken TblSil = db.tbl_ilan_degisken.First(a => a.id == ID);
        ...
        protected void btn_sabit_adi_ekle_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Class.Fonksiyonlar.JavaScript.MesajKutusuVeYonlendir("İlan değişken kategorisi başarıyla eklenmiştir. İlan değişkenleri listesine yönlendiriliyorsunuz.", "ilan-degisken.aspx");
            }
            catch (Exception ex)
            {
