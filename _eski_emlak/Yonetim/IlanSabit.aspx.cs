...
public partial class Yonetim_Kullanici : System.Web.UI.Page
{
        ...
        using (BaglantiCumlesi db = new BaglantiCumlesi())
        {
            var SQL = (from a in db.ilan_sabit2
                       ...
                       select new
                       {
                           ...
                           TipDegisken = db.ilan_degisken.Where(b => b.ID == a.IlanDegiskenID).Select(b => b.Baslik).FirstOrDefault(),
                           a.KayitTarih,
                           a.DegisTarih,
    ...
    protected void kayitlar_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            e.Row.Cells[8].Text = "<a href=\"IlanSabitDuzenle.aspx?ID=" + e.Row.Cells[1].Text + "\"><img src=\"Image/komut-duzenle.png\" /></a>";
        }
    }
    ...
    protected void secilenlerisil_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow satir in kayitlar.Rows)
        {
            ...
            if (kutu.Checked)
            {
                ...
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    ilan_sabit2 TblKullanici = db.ilan_sabit2.First(a => a.ID == ID);
    ...
    {
        Response.Redirect("IlanSabitEkle.aspx");
    }
    ...
    protected void kayitlar_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Onay")
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL = from a in db.ilan_sabit2 where a.ID == ID select new { a.Onay };
                    ...
                    foreach (var item in SQL)
                    {
                        switch (item.Onay)
                        {
                            case 1:
                                ...
                                {
                                    ilan_sabit2 TblOnay = dbOnay.ilan_sabit2.First(a => a.ID == ID);
                                    TblOnay.Onay = 0;
        ...
        if (e.CommandName == "Sil")
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ilan_sabit2 TblSil = db.ilan_sabit2.First(a => a.ID == ID);
