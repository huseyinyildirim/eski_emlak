...
{
    public partial class olay_giris_cikis : System.Web.UI.Page
    {
        ...
        protected void Sayfalama()
        {
            ...
            {
                var SQL = db.tbl_olay_giris_cikis.Count();
        ...
        protected void KayitYukle(string SqlCumle)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL = (from a in db.tbl_olay_giris_cikis
                           ...
                           select new
                           {
                               ...
                               a.tarih_ek,
                               a.ip,
                               Kullanici = db.tbl_admin.Where(b => b.id == a.kullanici_id).Select(b => b.ad).FirstOrDefault() + " (" + db.tbl_admin.Where(b => b.id == a.kullanici_id).Select(b => b.eposta).FirstOrDefault() + ")"
        ...
        protected void secilenlerisil_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow satir in kayitlar.Rows)
            {
                ...
                if (kutu.Checked)
                {
                    ...
                    using (BaglantiCumlesi db = new BaglantiCumlesi())
                    {
                        tbl_olay_giris_cikis TblSil = db.tbl_olay_giris_cikis.First(a => a.id == ID);
                        db.DeleteObject(TblSil);
                        db.SaveChanges();
                    }
                    Yonetim.Olay.Islem("olay_giriscikis", "Silindi", ID.ToString());
                }
            }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL = from a in db.tbl_olay_giris_cikis select a;

                foreach (var item in SQL)
                {
                    db.tbl_olay_giris_cikis.DeleteObject(item);

                }

                db.SaveChanges();
            }
            Yonetim.Olay.Islem("olay_giris_cikis", "Silindi", "Hepsi");
