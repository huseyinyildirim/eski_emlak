...
public partial class Yonetim_Kullanici : System.Web.UI.Page
{
    ...
    protected void Sayfalama()
    {
        ...
        {
            var SQL = from a in db.sayac where a.Bot == 0 group a by a.IsletimSistemi into g select new { TarihSayi = g.Count() };
    ...
    protected void KayitYukle(string SqlCumle)
    {
        ...
        using (BaglantiCumlesi db = new BaglantiCumlesi())
        {
            var SQL = (from a in db.sayac
                       where a.Bot == 0
                       group a by a.IsletimSistemi into g
                       select new
                       {

                           IsletimSistemi = g.Key,
                           Hit = g.Count()
                       }).OrderBy(p => p.IsletimSistemi).Skip(Limit).Take(int.Parse(kayitsayi.Text));
            ...
            if (SQL.AsEnumerable().Count() > 0)
            {
                ...
                foreach (var item in SQL)
                {
                    Veri += "" + item.IsletimSistemi + "|" + item.Hit.ToString() + "";
                ...
                grafik.Text = Yonetim.JavaScript.GrafikPasta(Veri);
            }
        }
    }
