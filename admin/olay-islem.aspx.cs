...
{
    public partial class olay_islem : System.Web.UI.Page
    {
        ...
        protected void Sayfalama()
        {
            ...
            {
                var SQL = db.tbl_olay_islem.Count();
        ...
        protected void KayitYukle(string SqlCumle)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL = (from a in db.tbl_olay_islem
        ...
        protected void secilenlerisil_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow satir in kayitlar.Rows)
            {
                ...
                if (kutu.Checked)
                {
                    ...
                    using (BaglantiCumlesi db = new BaglantiCumlesi())
                    {
                        tbl_olay_islem TblSil = db.tbl_olay_islem.First(a => a.id == ID);
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL = from a in db.tbl_olay_islem select a;

                foreach (var item in SQL)
                {
                    db.tbl_olay_islem.DeleteObject(item);
                }
