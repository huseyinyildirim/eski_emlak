...
public partial class Yonetim_Kullanici : System.Web.UI.Page
{
    ...
    protected void Sayfalama()
    {
        ...
        using (BaglantiCumlesi db=new BaglantiCumlesi())
        {
            var SQL = db.ilan_ozellik.Where(b=>b.UstID==0).Count();
    ...
    protected void kayitlar_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            e.Row.Cells[5].Text = "<a href=\"IlanOzellikDuzenle.aspx?ID=" + e.Row.Cells[1].Text + "\"><img src=\"Image/komut-duzenle.png\" /></a>";
        }
    }
    ...
    protected void secilenlerisil_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow satir in kayitlar.Rows)
        {
            ...
            if (kutu.Checked)
            {
                ...
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    ilan_ozellik TblKullanici = db.ilan_ozellik.First(a => a.ID == ID);
    ...
    protected void kayitlar_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Onay")
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL = from a in db.ilan_ozellik where a.ID == ID select new { a.Onay };
                    ...
                    foreach (var item in SQL)
                    {
                        switch (item.Onay)
                        {
                            case 1:
                                ...
                                {
                                    ilan_ozellik TblOnay = dbOnay.ilan_ozellik.First(a => a.ID == ID);
                                    TblOnay.Onay = 0;
        ...
        if (e.CommandName == "Sil")
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ilan_ozellik TblSil = db.ilan_ozellik.First(a => a.ID == ID);
    ...
    protected void btn_sabit_adi_ekle_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Class.Fonksiyonlar.JavaScript.MesajKutusuVeYonlendir("İlan özellik kategorisi başarıyla eklenmiştir. İlan özellikleri listesine yönlendiriliyorsunuz.", "IlanOzellik.aspx");
        }
        catch (Exception ex)
        {
            Class.Fonksiyonlar.JavaScript.MesajKutusuVeYonlendir(Yonetim.Degiskenler.Hata + ex.Message, "IlanOzellik.aspx");
        }
    }
}