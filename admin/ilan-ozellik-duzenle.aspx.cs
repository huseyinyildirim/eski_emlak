...
{
    public partial class ilan_ozellik_duzenle : System.Web.UI.Page
    {
        ...
        protected void btn_kayitekle_Click(object sender, EventArgs e)
        {
            if (Request.QueryString["ID"] != null && Class.Fonksiyonlar.Genel.NumerikKontrol(Request.QueryString["ID"].ToString()))
            {
                ...
                try
                {
                    using (BaglantiCumlesi db = new BaglantiCumlesi())
                    {
                        tbl_ilan_ozellik TblEkle = db.tbl_ilan_ozellik.First(a => a.id == ID);
                    ...
                    Class.Fonksiyonlar.JavaScript.MesajKutusuVeYonlendir("İlan özelliği başarıyla düzenlenmiştir.", "ilan-ozellik-duzenle.aspx?ID=" + ID + "");
                }
                catch (Exception ex)
                {
