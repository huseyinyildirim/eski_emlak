...
public partial class Yonetim_Kullanici : System.Web.UI.Page
{
    ...
    protected void Sayfalama()
    {
        ...
        {
            var SQL = db.olay_girishata.Count();
    ...
    protected void KayitYukle(string SqlCumle)
    {
        ...
        using (BaglantiCumlesi db = new BaglantiCumlesi())
        {
            var SQL = (from a in db.olay_girishata
    ...
    protected void secilenlerisil_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow satir in kayitlar.Rows)
        {
            ...
            if (kutu.Checked)
            {
                ...
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    olay_girishata TblSil = db.olay_girishata.First(a => a.ID == ID);
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        using (BaglantiCumlesi db=new BaglantiCumlesi())
        {
            var SQL = from a in db.olay_girishata select a;

            foreach (var item in SQL)
            {
                db.olay_girishata.DeleteObject(item);
                
            }
            
            db.SaveChanges();
        }
        Yonetim.Olay.Islem("olay_girishata", "Silindi", "Hepsi");
