...
public partial class Yonetim_Kullanici : System.Web.UI.Page
{
        ...
        using (BaglantiCumlesi db = new BaglantiCumlesi())
        {
            var SQL = (from a in db.sayfa
                       select new
                       {
                           ...
                           a.Baslik,
                           a.Onay,
                           Kategori = db.sayfa.Where(b => b.ID == a.UstID).Select(b => b.Baslik).FirstOrDefault(),
                           a.KayitTarih,
                           a.DegisTarih,
    ...
    protected void kayitlar_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            e.Row.Cells[6].Text = "<a href=\"SayfaDuzenle.aspx?ID=" + e.Row.Cells[1].Text + "\"><img src=\"Image/komut-duzenle.png\" /></a>";

            int ID = int.Parse(e.Row.Cells[1].Text);
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL = from a in db.sayfa where a.UstID == 0 && a.ID == ID select a;
    ...
    protected void secilenlerisil_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow satir in kayitlar.Rows)
        {
            ...
            if (kutu.Checked)
            {
                ...
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    sayfa TblKullanici = db.sayfa.First(a => a.ID == ID);
    ...
    protected void kayitlar_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Onay")
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL = from a in db.sayfa where a.ID == ID select new { a.Onay };
                    ...
                    foreach (var item in SQL)
                    {
                        switch (item.Onay)
                        {
                            case 1:
                                ...
                                {
                                    sayfa TblOnay = dbOnay.sayfa.First(a => a.ID == ID);
                                    TblOnay.Onay = 0;
        ...
        if (e.CommandName == "Sil")
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                sayfa TblSil = db.sayfa.First(a => a.ID == ID);
